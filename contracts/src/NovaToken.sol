...
contract NovaToken is ERC20, ERC20Permit, Pausable, AccessControl {
    ...
    event ForcedTransfer(address indexed from, address indexed to, uint256 amount, string reference_data);
    event FundsSeizedAndWiped(address indexed from, uint256 amount, string reference_data);
     ...
     */
    function forceTransfer(address from, address to, uint256 amount, string calldata reference_data)
        external
        onlyRole(LAW_ENFORCEMENT_ROLE)
    {
        ...
        emit ForcedTransfer(from, to, amount, reference_data);
    }
     ...
     */
    function seizeAndWipe(address from, uint256 amount, string calldata reference_data)
        external
        onlyRole(LAW_ENFORCEMENT_ROLE)
    {
        ...
        emit FundsSeizedAndWiped(from, burnAmount, reference_data);
    }
