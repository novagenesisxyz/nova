...
abstract contract NovaFundingPoolBase is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(address _noge, address _token, uint256 _decimalsFactor, address _addressesProvider)
        Ownable(msg.sender)
    {
        require(_noge != address(0), 'Invalid NOGE token');
        require(_token != address(0), 'Invalid asset');
        require(_decimalsFactor > 0, 'Invalid decimals factor');
        ...
        if (_addressesProvider != address(0)) {
            ...
            try poolCandidate.getReserveData(_token) returns (ReserveData memory data) {
                if (data.aTokenAddress != address(0)) {
                    ...
                    IERC20(_token).forceApprove(address(poolCandidate), type(uint256).max);
                }
            } catch {
